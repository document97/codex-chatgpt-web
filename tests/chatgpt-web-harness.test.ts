...
describe("ChatGPT outer-native harness v4", () => {
  ...
  test("does not wait for another answer when a tool batch is observed after the final DOM completed", () => {
    const state = {
      responsePresent: true,
      running: false,
      currentText: "final answer already rendered",
      currentHtml: "<p>final answer already rendered</p>",
      completionActionVisible: true,
      externalToolCallsInFlight: false,
    };
    const tracker = new ChatGptCompletionTracker(2_000, 60_000);
    expect(tracker.observeToolBatch(1, state.currentText, true)).toBe(true);
    expect(tracker.update(state, 1_000)).toBe(false);
    expect(tracker.update(state, 2_999)).toBe(false);
    expect(tracker.update(state, 3_000)).toBe(true);
  });

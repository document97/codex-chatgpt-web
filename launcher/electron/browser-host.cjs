...
class BrowserHost {
  ...
  async logout() {
    ...
    return await this.withManualOperation("ChatGPT logout", async () => {
      ...
      // Electron's test doubles and older embedded sessions may expose storage clearing without
      // the cache method. Storage removal is the authentication boundary; cache clearing is best
      // effort and must not turn logout into a failed operation.
      if (typeof contents.session.clearCache === "function") {
        await contents.session.clearCache();
      }
      if (typeof contents.session.clearCodeCaches === "function") {
        await contents.session.clearCodeCaches({ urls: [] });
      }
      this.setState({
        authenticated: false,
        loading: true,
